...
import in.loanwiser.partnerapp.BankStamentUpload.ApiConfig;
import in.loanwiser.partnerapp.BankStamentUpload.Appconfig;
...
import in.loanwiser.partnerapp.BankStamentUpload.BanklistAdapter;
import in.loanwiser.partnerapp.BankStamentUpload.FileAdapter;
import in.loanwiser.partnerapp.BankStamentUpload.Statementlist;
...
public class Upload_Activity_Bank_Old extends SimpleActivity implements  View.OnClickListener, CompoundButton.OnCheckedChangeListener, SingleUploadBroadcastReceiver.Delegate, PickiTCallbacks {

    private static final int PICK_PDF_REQUEST = 1;
    private static final String TAG = Upload_Activity_Bank_Old.class.getSimpleName() ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Objs.a.setStubId(this, R.layout.activity_bank_statement_old);
        ...
        statementlist = new Statementlist(Upload_Activity_Bank_Old.this, items);
        bankstatement_recycleview.setLayoutManager(new LinearLayoutManager(Upload_Activity_Bank_Old.this, LinearLayoutManager.VERTICAL, false));
        ...
        view_analysisbut.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent i = new Intent(Upload_Activity_Bank_Old.this, BankAnalysis_old.class);
              //  Intent in=new Intent(context,BankAnalysis_old.class);
        ...
        if (requestCode == PICK_PDF_REQUEST && resultCode == RESULT_OK) {
            ...
            if (data.getClipData() != null) {
                ...
                for (int i = 0; i < totalItemsSelected; i++) {
                    ...
                    Log.i("Upload_Bank_Old", "onActivityResult_fileuri: " + fileUri);
                    Log.i("Upload_Bank_Old", "onActivityResult_path: " + fileget);
                    Log.i("Upload_Bank_Old", "ActivityResult_filename: " + fileName);
                }
            } else {
            ...
            case R.id.submit:
                if (checkradiobutton_value!=null) {
                    if (uriarrayList.isEmpty()) {
                        Toast.makeText(Upload_Activity_Bank_Old.this, "Please Select Bank statement", Toast.LENGTH_SHORT).show();
                    } else if (uriarrayList.size() == 0) {
                        Toast.makeText(Upload_Activity_Bank_Old.this, "Please upload document", Toast.LENGTH_SHORT).show();
                    } else {
                        //    uploadMultipart();
                        ...
                        for (int i = 0; i < uriarrayList.size(); i++) {
                            ...
                            if (pdf1.equals(a)) {
                               ...
                               /* if (SDK_INT < 11) {
                                    orginalFile = new File(FileUtils1.getRealPathFromURI_BelowAPI11(Upload_Activity_Bank_Old.this, uriarrayList.get(i)));
                                }
                                // SDK >= 11 && SDK < 19
                                else if (SDK_INT < 19) {
                                    orginalFile = new File(FileUtils1.getRealPathFromURI_API11to18(Upload_Activity_Bank_Old.this, uriarrayList.get(i)));
                                }
                                // SDK > 19 (Android 4.4) and up
                                else {
                                    Log.e("path", "api 19 called");
                                    orginalFile = new File(FileUtils1.getRealPathFromURI_API19(Upload_Activity_Bank_Old.this, uriarrayList.get(i)));
...
*/
    ...
    private void Loan_submit_statues() {
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.POST, Urls.bank_status_fetch_old, J,
    ...
    public void Bank_statues() {
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.POST, Urls.update_bankstatementstatus_old, J,
                new Response.Listener<JSONObject>() {
                            ...
                            if (statues.contains("success")) {
                               // Applicant_Status();
                                ...
                                if(eligible_status.equals("0"))
                                {
                                 ...
                                 //   Toast.makeText(Upload_Activity_Bank_Old.this, "Bank Statement Analysis Failed", Toast.LENGTH_SHORT).show();
                                }else
                                {
                                    Intent intent = new Intent(Upload_Activity_Bank_Old.this, BankAnalysis_old.class);
                                    //  Intent in=new Intent(context,BankAnalysis_old.class);
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                Toast.makeText(Upload_Activity_Bank_Old.this, error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        }) {
            @Override
            public Map<String, String> getHeaders() throws AuthFailureError {
    ...
    private void BankverticalList() {
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.POST, Urls.bank_statementlist_old, J,
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject object) {
                        ...
                        try {
                            ...
                            RadioButton button = new RadioButton(Upload_Activity_Bank_Old.this);
                                   ...
                                   // String entity_id1 = J.getString("entity_id");
                                    String acc_number = J.getString("acc_number");
                                                  /*  Log.i(TAG, "onResponse:entity_id "+entity_id);
                                                    RadioButton rdbtn = new RadioButton(Upload_Activity_Bank_Old.this);
                                                    rdbtn.setId(View.generateViewId());
                                                    rdbtn.setText(entity_id + rdbtn.getId());
                                                    //rdbtn.setOnClickListener(this);
                                                    Bank_Statement_radio_list.addView(rdbtn);*/
                                    RadioButton button1 = new RadioButton(Upload_Activity_Bank_Old.this);
    ...
    private void MultifileUploadRetrofit() {
        ...
        Call<UploadFileResponse_old> call =getResponse.submitNew_old("Auth Token", fileParts,transaction_id1,analysis_status1,workorder_id1,entity_id1,relationship_type1,pdf_password1);
        call.enqueue(new Callback<UploadFileResponse_old>() {
            @Override
            public void onResponse(Call<UploadFileResponse_old> call, retrofit2.Response<UploadFileResponse_old> response) {
                progressDialog.dismiss();
                if (response.isSuccessful()){
                    UploadFileResponse_old response1=(UploadFileResponse_old) response.body();
                    ...
                    }else if(status==3)
                    {
                        ...
                        ErrorStatus1();
                       // Toast.makeText(Upload_Activity_Bank_Old.this,bankstatement_msg,Toast.LENGTH_SHORT).show();
            ...
            @Override
            public void onFailure(Call<UploadFileResponse_old> call, Throwable t) {
                progressDialog.dismiss();
                Toast.makeText(Upload_Activity_Bank_Old.this,"Network issue",Toast.LENGTH_SHORT).show();
                        ...
                        try {
                            ...
                            if (ja!=null && ja.length() > 0){
                                ...
                                for(int i = 0;i<ja.length();i++){
                                    ...
                                    recyclerView.setLayoutManager(new LinearLayoutManager(Upload_Activity_Bank_Old.this, LinearLayoutManager.HORIZONTAL, false));
                                    recyclerView.setHasFixedSize(true);
                                    banklistAdapter = new BanklistAdapter(Upload_Activity_Bank_Old.this, items1);
    ...
    private void Bank_analysis_ErrorStatus() {
        ...
        submitbtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent intent = new Intent(Upload_Activity_Bank_Old.this,Home.class);
                startActivity(intent);
                finish();
            }
        });
    ...
    private void MultifileUploadRetrofitGlib(){
        ...
        RequestBody pdf_password1 = RequestBody.create(MediaType.parse("multipart/form-data"), pdf_password);
        RequestBody relationship_type1 = RequestBody.create(MediaType.parse("multipart/form-data"), Pref.getCoAPPAVAILABLE(getApplicationContext()));
        Call<GlibResponse> call =getResponse.submitNews_old("Auth Token", fileParts,transaction_id1,analysis_status1,workorder_id1,entity_id1,relationship_type1,pdf_password1);
                    ...
                    public void onResponse(JSONObject object) {
                        ...
                        try {
                            ...
                            if (status==3){
                                ...
                                progressDialog.dismiss();
                                Toast.makeText(Upload_Activity_Bank_Old.this,"Success",Toast.LENGTH_SHORT).show();
                                Submit_upload_sucess();
                        /*    Intent intent=new Intent(Upload_Activity_Bank_Old.this,BankAnalysis_old.class);
